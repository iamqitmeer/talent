...
export async function addProjectToResume(
  ...
  try {
    ...
    const project = await Resume.findOne({ resumeId: resumeId });
    ...
    project.project = projectIds;

    const updatedProject = await project.save();

    return { success: true, data: JSON.stringify(updatedProject) };
  } catch (error: any) {
    console.error("Error adding or updating project to project: ", error);
    return { success: false, error: error?.message };
  }
}
